...
number_value = [value
                ...
                if df_train[
                    value].dtypes != 'O']  ##тип данных "O" представляет объект, который включает категориальные переменные
...
discret_values = [value for value in number_value if
                  len(data[value].unique()) < 25 and value not in for_year_value + ['Id']]
